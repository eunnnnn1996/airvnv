...
$(function(){
	...
	$(document).on('click','.delete-btn',function(){
		...
		//댓글 번호
		let re_num = $(this).attr('data-num'); /*data-num은 input에 설정한 사용자 속성*/
		
		let choice = confirm('댓글을 삭제하시겠습니까?'); /*confirm 사용시 확인 : true 반환 , 취소 : false 반환*/
		if(choice){
			$.ajax({
				type:'post',
				url:'replyDelete.do',
	...
	//1. 초기데이터(목록) 호출
	selectData(1, $('#rate_num').val());	//form에 qna_num이 hidden값으로 들어가있음
