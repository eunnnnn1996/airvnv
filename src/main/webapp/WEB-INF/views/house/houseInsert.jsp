...
<div class="market-main">
	...
	<div class="market-content-first">
		<p><b>1.</b> 전/월세 매물만 등록할 수 있습니다.</p>
		<p><b>2.</b> 글 등록 후 관리자의 승인 후 보여집니다.</p>
		<p><b>3.</b> 3~5일 후 승인</p>
		<p><b>4.</b> 1개의 매물만 등록 가능하며, 살아방에서 직거래로 표시됩니다.</p>
	</div>
	<div class="market-content-second">
		<div class="market-title2"><b>매물 종류</b></div>
	</div>
	
	<form:form modelAttribute="houseVO" action="houseInsert.do" enctype="multipart/form-data">
	...
	<div class="market-content-third">
		...
		<div class="third-radio">
			<label class="radio-label1"><span>&nbsp&nbsp&nbsp원&nbsp룸</span>
			<form:radiobutton onclick="test()" class="type-btn" path="market_type" value="1"/>
			</label>
			<label class="radio-label2"><span>&nbsp&nbsp&nbsp투&nbsp룸</span>
			<form:radiobutton onclick="test()" class="type-btn" path="market_type" value="2"/>
			</label>
			<label class="radio-label3"><span style="padding-left:15px">&nbsp쓰리룸</span>
			<form:radiobutton onclick="test()" class="type-btn" path="market_type" value="3"/>
			</label>
			<label class="radio-label4"><span style="padding-left:3px">오피스텔</span>
			<form:radiobutton onclick="test()" class="type-btn" path="market_type" value="4"/>
			</label>
			<label class="radio-label5"><span style="margin-left:15px">&nbsp아파트</span>
	...
	<hr class="title-hr">
		...
		<li>
			...
			<input type="button" class="btn-black postcode" onclick="sample2_execDaumPostcode()" value="우편번호 찾기">
		...
		<li>
			<form:input path="address2" placeholder="동"/><br>
			<form:errors path="address2" cssClass="error-color"/>
		</li>
		<li>
			<form:input path="address3" placeholder="호수"/><br>
		...
		<div class="third-radio">
			<label class="radio-label1"><span>&nbsp&nbsp&nbsp월&nbsp세</span>
			<form:radiobutton onclick="test()" class="type-btn" path="trade_type" value="1"/>
			</label>
			<label class="radio-label1"><span>&nbsp&nbsp&nbsp전&nbsp세</span>
			<form:radiobutton onclick="test()" class="type-btn" path="trade_type" value="1"/>
			</label>
			<form:checkbox path="trade_short" value="1"/> 단기가능
			<form:input class="otherpay-sty" path="otherpay" placeholder="관리비"/><br>
		...
		<div class="market-content-third">
			<div class="third-title"><span class="sort-choice"><div class="sort-content">거래 종류</div></span></div>
			<form:checkbox path="elevator" value="1"/> 엘리베이터
			<form:checkbox path="parking" value="1"/> 주차
			<form:checkbox path="veranda" value="1"/> 베란다
			옵션 기타설명 <form:textarea path="optionitem" cols="50" rows="5"/>
		</div>
...
<script>
/* $(function(){
	if($('input:radio[name="market_type"]:not(:checked)')){
		$("[class*='radio-label']").css('background-color','blue');
	}
}); */
  	 function test() {
        var obj_value = $("input:radio[name='market_type']:checked").val();     
        $('.radio-label'+obj_value).css('background-color','red');
    }  	 
